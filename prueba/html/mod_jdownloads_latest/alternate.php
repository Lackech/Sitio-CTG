...
defined('_JEXEC') or die;
    ...
    if ($files){
        $html = '<div class="moduletable box-parent" style="padding: 5px;">';
        ...
        for ($i=0; $i<count($files); $i++) {
			...
			// add mime file pic //Icono por defecto
            $size = 0;
			$files_pic = '';
			$number = '';
			if ($view_pics){
				$size = (int)$view_pics_size;
				$files_pic = '<img class ="icon-box" src="'.JURI::base().'images/jdownloads/fileimages/'.$files[$i]->file_pic.'" width="30%;" style="border: 0px; vertical-align: top;"'.' alt="" /> ';
            ...
            // add description in tooltip // mover todo el icon
            if ($view_tooltip && $files[$i]->description != ''){
              ...
              $link_text = '<a class ="titulo-box" href="'.$link.'">'.$files[$i]->file_title.' '.$version.$files[$i]->release.'</a>';
              
              $link_title_desc = '<p class="title_desc">'.JText::_('Breve descripcion: ').'</p>';
              
              $link_desc = '<div class ="desc_text">'.substr($files[$i]->description,0,$view_tooltip_length).'</div>';
              
              $link_down = '<a class="fa fa-arrow-circle-down" href="'.$link.'">'.JText::_(' Descargar').'</a>';
              ...
              $link_desc = '<p class ="desc_text">'.JText::_('No hay descripcion disponible para este articulo').'</p>';
              
              $link_down = '<a class="fa fa-arrow-circle-down" href="'.$link.'">'.JText::_(' Descargar').'</a>';
              	
			}    
			$html .= '<div  class ="box">'.$number.$link_text.$files_pic.$link_title_desc.$link_desc.$link_down.'</div>';
       ...
       $boton = '<a class="customLinkDocs" href="/index.php/documentos">'.JText::_('Ver todos').'</a>';
      
       $socket = '<div class ="boton-documentos">'.$boton.'</div>'; 
