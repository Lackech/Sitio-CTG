    ...
    if ($subheader != ''){
        ...
        if ($jlistConfig['view.subheader']) {
            ...
            if ($this->state->get('only_uncategorised')){
                ...
                $subheader = str_replace('{subheader_title}', JText::_('Pruebas'), $subheader);
            }    
    ...
    if ($layout_files_text != ''){
        ...
        for ($i = 0; $i < count($files); $i++) {
            ...
            if ($i > 0){
                ...
                $html_file = str_replace('{files_title_text}', JText::_('Archivos'), $html_file);//Titulo de todos los archivos
        ...
        if (isset($user_can_see_download_url)){ 
            ...
            if ($total_downloads > 0){
                $body = str_replace('{checkbox_top}', '<div class="jd_no_permission_downloads" style="text-align:center;"><img src="'.JURI::base().'components/com_jdownloads/assets/images/info32.png" style="text-align:middle;border:0px;" width="32" height="32" alt="info" /> '.$regg.'</div>', $body);                    
            } else {
                $body = str_replace('{checkbox_top}', '', $body);                    
            }    
        } 
